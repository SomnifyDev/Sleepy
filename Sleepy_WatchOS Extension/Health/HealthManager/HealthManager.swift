...
extension HealthManager {
	func subscribeToHeartBeatChanges() {
		...
		guard
			let sampleType: HKSampleType = HKObjectType.quantityType(forIdentifier: .heartRate) else
		{
			return
		}
	...
	private func startHeartRateQuery(quantityTypeIdentifier _: HKQuantityTypeIdentifier) {
		...
		let query = HKSampleQuery(
			...
			guard
				...
				let unit = HealthType.heart.HUnit else
			{
				return
			}
...
extension HealthManager {
	...
	private func makeHealthKitRequest() {
		guard
			...
			let activeEnergyBurned = HealthType.activeBurnedEnergy.HSample else
		{
			return
		}
