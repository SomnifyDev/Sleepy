...
public class HKSleepAppleDetectionProvider: HKDetectionProvider {
    ...
    public func saveSleep(sleep: Sleep, completionHandler: @escaping (Bool, Error?) -> Void) {
        ...
        hkService?.readData(type: .asleep, interval: expandedInterval, bundlePrefixes: ["com.benmustafa", "com.sinapsis"], completionHandler: { _, samples, _ in
    ...
    public func retrieveData(completionHandler: @escaping (Sleep?) -> Void) {
        ...
        self.getRawData(interval: interval) { _, asleepRaw, error1, _, heartRaw, error2, _, energyRaw, error3, _, inBedRaw, error4,
