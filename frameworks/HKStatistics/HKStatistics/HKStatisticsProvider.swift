...
public final class HKStatisticsProvider: HKStatistics {
	...
	public func getDataByIntervalWithIndicator(healthType: HKService.HealthType,
	                                           ...
	                                           completion: @escaping (Double?) -> Void)
	{
		self.healthService.readData(type: healthType, interval: timeInterval, ascending: true, bundlePrefixes: bundlePrefixes) { [weak self] _, sleepData, _ in
	...
	public func getMetaDataByIntervalWithIndicator(healthType: HKService.HealthType,
	                                               ...
	                                               completion: @escaping (Double?) -> Void)
	{
		self.healthService.readMetaData(key: healthType.metaDataKey, interval: timeInterval, ascending: true) { _, data, _ in
			completion(data)
		}
	}
	...
	public func getDataByInterval(healthType: HKService.HealthType,
	                              ...
	                              completion: @escaping ([Double]) -> Void)
	{
		self.healthService.readData(type: healthType, interval: timeInterval, ascending: true, bundlePrefixes: bundlePrefixes) { [weak self] _, samples, _ in
			switch healthType {
			case .energy:
