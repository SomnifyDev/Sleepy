...
public struct MotivationCellView: View {
    ...
    private func getTypeColor(for type: HealthData) -> Color {
        switch type {
        case .heart:
            return colorProvider.sleepyColorScheme.getColor(of: .heart(.heartColor))
        case .energy:
            return colorProvider.sleepyColorScheme.getColor(of: .energy(.energyColor))
        case .sleep:
