...
struct HistoryListView: View {
    ...
    @State private var respiratoryHistoryStatsViewModel: RespiratoryHistoryStatsViewModel?
        ...
        GeometryReader { _ in
            ZStack {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    ...
                    } else if calendarType == .respiratory {
                        if let respiratoryHistoryStatsViewModel = respiratoryHistoryStatsViewModel {
    ...
    func extractContextStatistics() {
        ...
        case .heart:
            self.extractHeartDataIfNeeded()
        case .energy:
            self.extractEnergyDataIfNeeded()
        case .sleep:
            self.extractSleepDataIfNeeded(type: .asleep)
        case .inbed:
            self.extractSleepDataIfNeeded(type: .inbed)
        case .respiratory:
            self.extractRespiratoryDataIfNeeded()
        }
    }

    func extractSleepDataIfNeeded(type: HKService.HealthType) {
        if (type == .inbed ? self.inbedHistoryStatsViewModel != nil : self.asleepHistoryStatsViewModel != nil) ||
    ...
    func extractHeartDataIfNeeded() {
        if self.heartHistoryStatsViewModel != nil {
            return
        }
    ...
    func extractRespiratoryDataIfNeeded() {
        if self.heartHistoryStatsViewModel != nil {
            return
        }
        ...
        group.enter()
        DispatchQueue.global(qos: .userInitiated).async {
            viewModel.statisticsProvider.getDataByIntervalWithIndicator(healthType: .respiratory,
                                                                        indicatorType: .min,
                                                                        for: monthBeforeDateInterval) { result in
                if let result = result {
                    last30daysCellData.append(StatisticsCellData(title: "Min. value", value: "\(Int(result))"))
                }
                group.leave()
            }
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            viewModel.statisticsProvider.getDataByIntervalWithIndicator(healthType: .respiratory,
                                                                        ...
                                                                        for: monthBeforeDateInterval) { result in
                if let result = result {
                    last30daysCellData.append(StatisticsCellData(title: "Avg. value", value: "\(Int(result))"))
                }
                group.leave()
            }
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            viewModel.statisticsProvider.getDataByIntervalWithIndicator(healthType: .respiratory,
                                                                        ...
                                                                        for: monthBeforeDateInterval) { result in
                if let result = result {
                    last30daysCellData.append(StatisticsCellData(title: "Max. value", value: "\(Int(result))"))
                }
                group.leave()
            }
        }
    ...
    func extractEnergyDataIfNeeded() {
        if self.energyHistoryStatsViewModel != nil {
            return
        }
