...
extension HistoryCoordinator
{
    ...
    func extractSleepDataIfNeeded(type: HKService.HealthType)
    {
        ...
        { [weak self] in
            ...
            self.statisticsProvider.getDataByIntervalWithIndicator(
                healthType: type,
                indicatorType: .mean,
        ...
        { indicator in
            ...
            { [weak self] in
                ...
                { result in
                    ...
                    {
                        last30daysCellData.append(StatisticsCellData(
                            title: self.getStatisticsCellDataLabel(for: type, indicator: indicator),
        ...
        if type == .asleep
        {
            ...
            { [weak self] in
                ...
                self.statisticsProvider.getDataByInterval(
                    healthType: type,
                    for: self.monthBeforeDateInterval,
                    bundlePrefixes: ["com.sinapsis", "com.benmustafa"]
                )
                ...
                DispatchQueue.main.async
                {
                    let tmp = SleepHistoryStatsViewModel(
                        ...
                        currentWeeksProgress: ProgressItem(
                            title: String(format: "Mean duration:", Date.minutesToDateDescription(minutes: Int(mean1))),
                            text: current2weeksInterval.stringFromDateInterval(type: .days),
                            value: Int(mean1)
                        ),
                        beforeWeeksProgress: ProgressItem(
                            title: String(format: "Mean duration:", Date.minutesToDateDescription(minutes: Int(mean2))),
